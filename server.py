        ...
        try:
            ...
            if message == "agent":
                ...
                if choice == 1:
                    ...
                    # Verifica se o número de réplicas atual é maior do que o número solicitado de réplicas
                    replicas_directory = os.path.join(path, "data")
                    current_replicas = len(
                        [
                            f
                            for f in os.listdir(replicas_directory)
                            if f.startswith(file_name + ".replica-")
                        ]
                    )
                    print(f"Número de réplicas: " + str(current_replicas))

                    if current_replicas > level:
                        replicas_to_remove = current_replicas - level
                        for i in range(
                            current_replicas - 1,
                            current_replicas - replicas_to_remove - 1,
                            -1,
                        ):
                            # print(f"Removendo réplica " + {i})
                            replica_path = os.path.join(
                                replicas_directory, file_name + f".replica-{i + 1}"
                            )
                            os.remove(replica_path)
                        output = f"{replicas_to_remove} Réplicas removidas com sucesso"
                    ...
                    if level > 0 and current_replicas < level:
            ...
            print(f"ERRO: Ocorreu uma exceção - {str(e)}")
            client.close()
            break
